...
void HandleNewPulseInterval(const PulseRpmSubSensorContext &config,
  ...
  if (averageInterval > 0U && intervalMicros > averageInterval + (averageInterval / 2U)) {
    const uint32_t missedPulses =
        (intervalMicros + (averageInterval / 2U)) / averageInterval;

    if (missedPulses > 1U && missedPulses <= 10U) {
      const uint32_t recoveredInterval = intervalMicros / missedPulses;
      const bool matchesRecovered = IsWithinPercentWindow(
          recoveredInterval, averageInterval, kPulseRpmOutlierPercent);
      ...
      if (matchesRecovered) {
        ...
        UpdateAcceptedRpm(config, runtime, recoveredInterval);
        return;
      }
    }
  }
